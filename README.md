...
* [courseR](https://github.com/whitwort/courseR) is the backend R script that builds the html pages from markdown source (see `/source`).  It uses [knitr](https://github.com/yihui/knitr) to handle literate programming, [highlight](http://cran.r-project.org/web/packages/highlight/index.html) to handle html class annotation on R code fragments, and [markdown](http://cran.r-project.org/web/packages/markdown/) to compile the rest of the document.
