...
# Preface on coding style

## Using functions to organize code

R features a very clear and relatively unabiguous syntax.  This means that you, as an author of R code, have a lot of leeway to format your code in any way you see fit.  Please use this feature of the language for good, not evil!
...
Please take a moment to read over [Hadley Wickham's recommended R style guide](http://adv-r.had.co.nz/Style.html), which is roughly equivalent to the [Google R Style guide](https://google-styleguide.googlecode.com/svn/trunk/Rguide.xml) (for googlers).

You'll see there are a couple discrepencies between these two guides, and between these guides and other large R code bases.  I won't strictly enforce one style, but I do ask that you follow two of the most important:

1. ALWAYS be consistent.  For example, you may pick from any of the three variable naming conventions (`my.variable`, `my_variable`, or `myVariable`), but ALWAYS use the same convention throughout all of your code.

2. Keep each line of code to 80 characters or less; use newlines to break things up.  You can turn on an 80 character guide in RStudio from the Tools -> Global Options -> Code Editing pane.

Finally, I'll admit a style quirk that I picked up when submitting a few pull requests to a non-R code project a few years ago:  the [common-first style](https://www.npmjs.org/doc/misc/npm-coding-style.html).
...
If your list of arugments is long, or have complex expressions for their values, it is often nice to split each element onto it's own line, like (notice the extra spacing to make all of the "=" signs line up nicely (as noted in the Google style guide):
...
Which did you see first?  The comma-first style is even more useful when working with nested data structures; however it's use is most definitely still controversial (and it only works if you strictly adhere to the perfect vertical alignment strategy).  Feel free to use it, or the more common comma syntax, but as always be consistent!  
...
(Hint: check out the `nchar` function along with the arithematic modulo operator `%%`/)
...
If you call findORFs from annotateFrame remember that you'll want to convert lenths and startPositions from the amino acid coordinates to the correct DNA coordinates.
...
Refactor your annotateChromosome function so that it first finds and then removes (hint: gsub) all of the likely introns from the chromosomal sequence before attempting to annotate the ORFs. You might 
